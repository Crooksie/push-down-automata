...
describe("PushDownAutomata", () => {
  ...
  it("handles a single returning state", () => {
    ...
    const mockState: State = {
      exec: jest.fn(() => StateResult.PopStateReturn),
    };
    expect(pda.empty).toBe(true);
  ...
  it("handles a continuing state", () => {
    const pda = new PushDownAutomata();
    const mockState: State = {
      exec: jest.fn(() => StateResult.KeepStateReturn),
    };
    expect(pda.empty).toBe(true);
  ...
  it("handles a continuing state", () => {
    const pda = new PushDownAutomata();
    const returningState: State = {
      exec: jest.fn(() => StateResult.PopStateReturn),
    };
    const continuingState: State = {
      exec: jest.fn((stack: PushState) => {
  ...
  it("passes through its args", () => {
    ...
    const exec = jest.fn(
      (stack: PushState, a: { a: number }, d: { b: string; c: string }) => {
    ...
    const obj = { b: initalB, c: initialC };

    pda.pushState(mockState);
    pda.exec(initialA, obj);

    expect(exec).toBeCalled();
    expect(obj.b).toBe(expectedB);
    expect(obj.c).toBe(expectedC);
  });
});
